...
template CommandRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		multi_or[2][i].in[2] <== and[18][i].out;
		multi_or[2][i].in[3] <== and[20][i].out;
		...
		lt[13][i].in[1] <== 191;
		and[23][i] = AND();
		and[23][i].a <== lt[12][i].out;
		and[23][i].b <== lt[13][i].out;
		and[24][i] = AND();
		and[24][i].a <== states[i][14];
		and[24][i].b <== and[23][i].out;
		and[25][i] = AND();
		and[25][i].a <== states[i][15];
		and[25][i].b <== and[12][i].out;
		eq[22][i] = IsEqual();
		eq[22][i].in[0] <== in[i];
		eq[22][i].in[1] <== 128;
		eq[23][i] = IsEqual();
		...
		from_zero_enabled[i] <== MultiNOR(25)([states_tmp[i+1][1], states[i+1][2], states[i+1][3], states[i+1][4], states[i+1][5], states[i+1][6], states[i+1][7], states[i+1][8], states[i+1][9], states[i+1][10], states[i+1][11], states[i+1][12], states[i+1][13], states[i+1][14], states[i+1][15], states[i+1][16], states[i+1][17], states[i+1][18], states[i+1][19], states[i+1][20], states[i+1][21], states[i+1][22], states[i+1][23], states[i+1][24], states[i+1][25]]);
	...
	for (var i = 0; i <= num_bytes; i++) {
		is_accepted.in[i] <== states[i][25];
	}
	out <== is_accepted.out;
	signal is_consecutive[msg_bytes+1][3];
	is_consecutive[msg_bytes][2] <== 0;
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][25] * (1 - is_consecutive[msg_bytes-i][2]) + is_consecutive[msg_bytes-i][2];
		is_consecutive[msg_bytes-1-i][1] <== state_changed[msg_bytes-i].out * is_consecutive[msg_bytes-1-i][0];
		is_consecutive[msg_bytes-1-i][2] <== ORAnd()([(1 - from_zero_enabled[msg_bytes-i+1]), states[num_bytes-i][25], is_consecutive[msg_bytes-1-i][1]]);
	}
	// substrings calculated: [{(9, 9), (9, 10), (9, 11), (9, 12), (9, 13), (9, 14), (9, 15), (9, 16), (10, 9), (11, 10), (12, 10), (13, 10), (14, 12), (15, 12), (16, 12)}]
	signal prev_states0[15][msg_bytes];
	signal is_substr0[msg_bytes];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		 // the 0-th substring transitions: [(9, 9), (9, 10), (9, 11), (9, 12), (9, 13), (9, 14), (9, 15), (9, 16), (10, 9), (11, 10), (12, 10), (13, 10), (14, 12), (15, 12), (16, 12)]
		...
		prev_states0[3][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][9];
		prev_states0[4][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][9];
		prev_states0[5][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][9];
		prev_states0[6][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][9];
		prev_states0[7][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][9];
		prev_states0[8][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][10];
		...
		is_substr0[i] <== MultiOR(15)([prev_states0[0][i] * states[i+2][9], prev_states0[1][i] * states[i+2][10], prev_states0[2][i] * states[i+2][11], prev_states0[3][i] * states[i+2][12], prev_states0[4][i] * states[i+2][13], prev_states0[5][i] * states[i+2][14], prev_states0[6][i] * states[i+2][15], prev_states0[7][i] * states[i+2][16], prev_states0[8][i] * states[i+2][9], prev_states0[9][i] * states[i+2][10], prev_states0[10][i] * states[i+2][10], prev_states0[11][i] * states[i+2][10], prev_states0[12][i] * states[i+2][12], prev_states0[13][i] * states[i+2][12], prev_states0[14][i] * states[i+2][12]]);
